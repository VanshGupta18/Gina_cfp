...
The backend uses **PostgreSQL hosted on [Supabase](https://supabase.com/)**, not a local Postgres instance or a database named `talktodata`.

| Concern | What to use |
|--------|----------------|
| **Connection string** | Supabase **Project Settings → Database**. Put the URI in `DATABASE_URL` (backend env). Prefer the **Transaction pooler** connection string (host `…pooler.supabase.com`, port **6543**) for the Node `pg` pool unless you need session-pinned features (coordinate with Person A for `readonly_agent` / long-lived behavior). |
| **Migrations** | Run `migrations/*.sql` against the Supabase project, e.g. `psql "$DATABASE_URL" -f migrations/001_initial_schema.sql`, the Supabase CLI, or paste into the **SQL Editor**. |
| **pgvector** | Enable **pgvector** in **Database → Extensions** (or via `CREATE EXTENSION` in migration if allowed on your plan). |
| **Verification SQL** | Commands below use `psql "$DATABASE_URL"` — export the variable first (`export DATABASE_URL='postgresql://…'` in bash), or run the same statements in the **Supabase SQL Editor**. Default logical database name is typically `postgres`. |
| **Auth** | Supabase **Auth** JWTs are verified against the same project as the database (see `Backend_Master.md` Section 10). |

---

## Phase 0 — Project Scaffold + Database

**Goal:** Runnable Fastify server + **Supabase** PostgreSQL with full schema. Zero business logic.
...
|---|---|---|
| **0** | Running server + full schema on Supabase Postgres | — |
...
```
Phase 0:  A ████████ scaffold + Supabase DB
          B ████████ server + env config
          ─────────── Gate 0 ───────────
